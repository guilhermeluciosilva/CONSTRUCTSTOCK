...
export const Layout: React.FC<{ children: React.ReactNode, onNavigate: (path: string) => void, currentPath: string }> = ({ children, onNavigate, currentPath }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      {/* Sidebar - Garantindo flex-col para controle de scroll */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transform transition-transform duration-300 lg:relative lg:translate-x-0 flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {/* Navigation - ÁREA COM SCROLL INDEPENDENTE */}
        <nav className="flex-1 px-4 py-6 overflow-y-auto custom-scrollbar space-y-8">
          {/* Fix: Explicitly casting Object.entries to ensure 'items' is recognized as an array type for .length and .map access */}
          {(Object.entries(categorizedItems) as [string, typeof activeMenuItems][]).map(([catId, items]) => {
            if (items.length === 0) return null;
            return (
              <div key={catId} className="space-y-2">
                {/* Rótulo de Categoria - Apenas se não for a Principal para economizar espaço */}
                {catId !== 'MAIN' && (
        ...
        {/* User & Logout Section - FIXA NO RODAPÉ */}
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm z-40 shrink-0">
          <div className="flex items-center gap-4 flex-1">
            <button className="lg:hidden text-slate-400 hover:text-slate-600" onClick={() => setIsMobileMenuOpen(true)}>
              <i className="fas fa-bars text-xl"></i>
            </button>
            
            <div className="flex items-center gap-4 text-sm overflow-x-auto no-scrollbar py-1">
              <div className="hidden sm:flex items-center gap-2 group shrink-0">
              ...
              {/* Seletor de Unidade */}
              <div className="flex items-center border-l border-slate-100 pl-4 shrink-0">
