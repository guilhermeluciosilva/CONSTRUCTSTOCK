...
export const SalesList: React.FC<{ onNew: () => void }> = ({ onNew }) => {
  const { currentScope, activeTenant } = useApp();
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm italic">Gestão de saídas diretas para consumidor final.</p>
        </div>
        <button 
          onClick={onNew}
          className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase shadow-lg shadow-emerald-200 flex items-center gap-2 hover:bg-emerald-700 transition-all"
        >
          <i className="fas fa-cart-plus"></i> Registrar Nova Venda
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400">
            <tr>
              <th className="px-8 py-5">Cód. Venda</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {sales.map(sale => (
              <tr key={sale.id} className="hover:bg-emerald-50/20 transition-all group text-sm">
                <td className="px-8 py-6 font-black text-emerald-600">{sale.id}</td>
                <td className="px-6 py-6 font-bold text-slate-800">{sale.customerName}</td>
                ...
                <td className="px-8 py-6 text-right">
                  <button className="w-9 h-9 rounded-xl bg-slate-50 text-slate-300 hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center">
