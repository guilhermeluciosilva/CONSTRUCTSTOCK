...
export const Dashboard: React.FC<DashboardProps> = ({ onSelectRM, onNavigate }) => {
  ...
  const stats = [
    ...
    { id: 'skus', label: 'SKUs em Estoque', value: inventoryStats.totalSkus, icon: 'fa-boxes-stacked', color: 'text-indigo-600', bg: 'bg-indigo-50', path: '/stock' },
  ...
  // Regra de Paywall (Exemplo baseado no plano FREE)
  const skuLimit = 300;
  const usagePercentage = (inventoryStats.totalSkus / skuLimit) * 100;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
            ...
            <button 
              ...
              title="Adicionar ou remover informações"
            >
              <i className="fas fa-plus-circle mr-1"></i> Aditivar
            </button>
          </h1>
        ...
        <div className={`p-6 rounded-[28px] border-2 transition-all duration-500 ${isHardLimit ? 'bg-rose-50 border-rose-200' : isSoftLimit ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-100 shadow-sm'}`}>
           <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="space-y-1">
                 <h4 className={`text-xs font-black uppercase tracking-widest ${isHardLimit ? 'text-rose-600' : isSoftLimit ? 'text-amber-600' : 'text-slate-400'}`}>Uso do Plano Atual (FREE)</h4>
                 <p className="text-sm font-bold text-slate-700">Você utilizou {inventoryStats.totalSkus} de {skuLimit} SKUs permitidos.</p>
              </div>
                ...
                movements.slice(0, 10).map(m => (
                  <div key={m.id} className="w-full px-8 py-5 flex items-center justify-between hover:bg-slate-50/50 transition-all group">
                     ...
                     <div className="text-right">
                        <p className={`font-black text-sm ${m.quantity > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{m.quantity > 0 ? '+' : ''}{m.quantity} un.</p>
      ...
      {/* Modal de Aditivar (Customização de Widgets) */}
      {showAditivarModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95">
            <div className="p-8 border-b bg-slate-50 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-black text-slate-800">Aditivar Dashboard</h2>
            ...
            <div className="p-8 space-y-4">
              ...
              <div className="space-y-3">
                {[
                  { id: 'stats', label: 'Resumo de Indicadores', desc: 'Cards superiores com totais de SKUs, Vendas e RMs.', icon: 'fa-chart-pie' },
                  ...
                  { id: 'usage', label: 'Monitoramento de Plano', desc: 'Acompanhe limites de SKUs e movimentações.', icon: 'fa-tachometer-alt' }
                ].map(widget => (
                  <button 
                    key={widget.id}
            ...
            <div className="p-8 bg-slate-50 border-t flex justify-end">
              <button 
                ...
                className="px-10 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-xs shadow-lg shadow-blue-500/20 hover:bg-blue-700 active:scale-95 transition-all"
              >
                Aplicar Mudanças
              </button>
            </div>
